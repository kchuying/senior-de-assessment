def data_applicant_cleaner():
    ...
    #Set variables
    fpath = "/usr/local/airflow/store_files_airflow/data_files/"
    ...
    error_df.to_csv('~/store_files_airflow/data_error_logs/error_data_log_{date}.csv'.format(date = yesterday_date, index=False))
    ...
    final_df.to_csv('~/store_files_airflow/results/cleansed_data_{date}.csv'.format(date = yesterday_date, index=False))
